...
def get_tric(tric):
    ...
    for par, msn in tric.collect():
        ...
        if len(list_msn) > 1 and 'exists' in list_msn:
            for i in list_msn:
                if i != 'exists':
                    ftrics.append((i[1], par[0], par[1]))
    return sorted(ftrics)


def main(sc, list_file):
    """
    Recorremos la lista de ficheros, para cada fichero creamos 
    el grafo mapeando y eliminando duplicados. Después creamos 
    la lista de nodos adyacentes ordenando por Key, obtenemos 
    los triciclos y luego imprimimos la lista resultante.
    """
    for file in list_file:
        ...
        print ('File ', file, ' has  these tricycles: ', get_tric(tric))
